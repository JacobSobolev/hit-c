...
namespace A17.Ex05.GameOthelloLogic
{
    ...
    public delegate void GameFinishEventHandler();
    ...
    public class Game
    {
        ...
        public event BoardEventHandler BoardChanged;

        public event GameFinishEventHandler GameFinished;
        ...
        public Game(int i_ValidBoardSize) : this(i_ValidBoardSize, k_Player1NameDefualt, k_Player2NameDefualt, k_Player2TypeDefualt)
        {
        }
        ...
        private void calcValidPlayerMoves(Player i_Player)
        {
            ...
            for (int i = 0; i < pointsToCheck.Count; i++)
            {
                ...
                validCurrentMoves = checkMoveDirection(pointsToCheck[i], eGameMoveDirectionType.Horizontal, getOpponentPlayerPiece());
                addValidGameMoves(validCurrentMoves, ref validPlayerMoves);
                validCurrentMoves = checkMoveDirection(pointsToCheck[i], eGameMoveDirectionType.Vertical, getOpponentPlayerPiece());
                addValidGameMoves(validCurrentMoves, ref validPlayerMoves);
                validCurrentMoves = checkMoveDirection(pointsToCheck[i], eGameMoveDirectionType.DiagonalLeftTopToBottomRight, getOpponentPlayerPiece());
                addValidGameMoves(validCurrentMoves, ref validPlayerMoves);
                validCurrentMoves = checkMoveDirection(pointsToCheck[i], eGameMoveDirectionType.DiagonalLeftBottomToTopRight, getOpponentPlayerPiece());
                addValidGameMoves(validCurrentMoves, ref validPlayerMoves);
            }
        ...
        private List<GameValidMoves> checkMoveDirection(Point i_Point, eGameMoveDirectionType i_DirectionToCheck, BoardCellType i_OpponentPlayerPiece)
        {
            ...
            if (i_DirectionToCheck == eGameMoveDirectionType.Horizontal)
            {
            ...
            else if (i_DirectionToCheck == eGameMoveDirectionType.Vertical)
            {
            ...
            else if (i_DirectionToCheck == eGameMoveDirectionType.DiagonalLeftTopToBottomRight)
            {
                backwardsMoveToCheck.Dx = -1;
                backwardsMoveToCheck.Dy = -1;
                forwardsMoveToCheck.Dx = 1;
        ...
        {
            return i_Point.X >= 0 && i_Point.X < m_Board.BoardSize && i_Point.Y >= 0 && i_Point.Y < m_Board.BoardSize;
        }
        ...
        {
            if (!IsGameOver() && m_CurrentPlayer.ValidMoves != null && m_CurrentPlayer.ValidMoves.Count > 0)
            {
        ...
        public void MakePcMove()
        {
            ...
            if (!IsGameOver() || ((!IsGameOver()) && (m_CurrentPlayer.ValidMoves == null || m_CurrentPlayer.ValidMoves.Count == 0)))
            {
                NextPlayer();
                calcValidPlayerMoves(m_Player1);
                calcValidPlayerMoves(m_Player2);

                if (m_CurrentPlayer.ValidMoves == null || m_CurrentPlayer.ValidMoves.Count == 0)
                {
        ...
        private List<GameValidMoves> findVaildGameMove(Point i_Point, Player i_Player)
        {
            ...
            {
                if (i_Player.ValidMoves[i].EndPoint.X == i_Point.X && i_Player.ValidMoves[i].EndPoint.Y == i_Point.Y)
                {
                    if (returnVaule == null)
                    {
        ...
        private void FlipPieces(List<GameValidMoves> i_ValidMoves, Player i_CurrentPlayer, Player i_OpponentPlayer)
        {
            ...
            {
                int indexI = i_ValidMoves[i].StartPoint.X;
                int indexJ = i_ValidMoves[i].StartPoint.Y;
                int endI = i_ValidMoves[i].EndPoint.X;
                int endJ = i_ValidMoves[i].EndPoint.Y;
        ...
        public bool IsGameOver()
        {
            ...
            if (m_Board.UnUsedCells == 0 || ((m_Player1.ValidMoves == null || m_Player1.ValidMoves.Count == 0) && (m_Player2.ValidMoves == null || m_Player2.ValidMoves.Count == 0)))
            {
                if (GetWinResult() > 0)
                {
                    m_Player1.IncGamesWon();
                }
                else if (GetWinResult() < 0)
                {
                    m_Player2.IncGamesWon();
                }
